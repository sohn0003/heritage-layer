...
const Mypage = () => {
  ...
  const stats = [
    { icon: Star, label: '관심 자산', value: savedAssets.length },
    { icon: Handshake, label: '딜 관심 신청', value: dealSignals.length },
    { icon: Building2, label: '진행중', value: dealSignals.filter((s) => s.admin_status === 'in_progress' || s.admin_status === 'pending').length },
  ];

  return (
    <div className="min-h-screen bg-background pt-24 pb-20">
      <Seo title="마이페이지 — Heritage Layer" description="저장한 매물과 딜 관심 신청 내역을 확인하세요." path="/mypage" />
      <div className="mx-auto max-w-4xl px-6 sm:px-8 space-y-10">
        {/* Header */}
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[11px] uppercase tracking-[0.25em] text-muted-foreground">My Dashboard</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight">마이페이지</h1>
        ...
        {isAdmin && (
          <Section icon={Inbox} title="관심 상담 신청 관리" action={
            ...
            <p className="text-sm leading-relaxed text-muted-foreground">사용자가 신청한 관심 상담 내역을 확인하고 처리하세요.</p>
          </Section>
        )}
        ...
        <Section icon={Star} title="관심 자산">
          {savedAssets.length === 0 ? (
            <div className="py-8 text-center text-sm text-muted-foreground">
              ...
              <Button variant="link" onClick={() => navigate('/properties')} className="mt-1">
            ...
            <ul className="divide-y divide-border/50">
              {savedAssets.map((item) => (
                <li key={item.id}
                  ...
                  <div>
                    <p className="text-sm font-medium leading-snug">{item.asset?.address || '주소 정보 없음'}</p>
              ...
              {dealSignals.map((s) => (
                <li key={s.id}
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-medium leading-snug">{s.asset?.address || '주소 정보 없음'}</p>
                    <span className={`shrink-0 text-[11px] px-2 py-0.5 border ${
                      ...
                      : s.admin_status === 'in_progress' ? 'border-accent/40 text-accent'
                  ...
                  {s.admin_response && (
                    <p className="mt-2 border-l-2 border-accent/40 pl-3 text-xs leading-relaxed text-muted-foreground">
        ...
        <Section icon={FileText} title="저장된 레포트">
          <p className="py-6 text-center text-sm text-muted-foreground">준비 중입니다.</p>
        </Section>
      </div>
    </div>
  );
