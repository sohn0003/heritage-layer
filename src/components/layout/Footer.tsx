...
const Footer = () => {
  ...
  return (
    <footer className="border-t bg-card">
      {/* 모바일 축약 푸터 */}
      <div className="mx-auto flex max-w-5xl flex-col items-center gap-2 px-4 py-5 text-center text-xs text-muted-foreground sm:hidden">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/about')} className="hover:text-foreground">회사 소개</button>
          <span className="opacity-40">·</span>
          <button onClick={() => navigate('/contact')} className="hover:text-foreground">문의</button>
          <span className="opacity-40">·</span>
          <button onClick={() => navigate('/terms')} className="hover:text-foreground">약관</button>
          <span className="opacity-40">·</span>
          <button onClick={() => navigate('/privacy')} className="hover:text-foreground">개인정보</button>
        </div>
        <p>© 2025 (주)더레이어코퍼레이션</p>
      </div>

      {/* 데스크톱/태블릿 풀 푸터 */}
      <div className="hidden px-4 py-14 sm:block">
